...
# [Keep all existing constants - COURSE_DESCRIPTIONS, YLO_STRUCTURE, etc.]
COURSE_DESCRIPTIONS = {
    '282711': {
...
class GoogleSheetsManager:
    """Manage Google Sheets operations for assessment data"""
            ...
            for sheet_name, headers in sheets_config.items():
                try:
                    ...
                    try:
                        ...
                        created_sheets.append(f"อัพเดต {sheet_name}")
    ...
    def save_assessment_data(self, spreadsheet, assessment_data, file_info=None):
        """Save assessment results to Google Sheets"""
        try:
            assessment_id = assessment_data.get('assessment_id', 
                                               datetime.now().strftime('%Y%m%d_%H%M%S'))
            
            # 1. Save to Assessment_Summary
            summary_sheet = spreadsheet.worksheet('Assessment_Summary')
            
            summary_row = [
                datetime.now().strftime('%Y-%m-%d'),  # วันที่
                datetime.now().strftime('%H:%M:%S'),  # เวลา
            ...
            content_row = [
                assessment_id,
                assessment_data['course_code'],
                assessment_data.get('content_hash', ''),
            ...
            return True, f"บันทึกข้อมูลการประเมิน {assessment_id} สำเร็จ"
...
# Google Sheets Interface Functions
def show_google_sheets_setup():
    """Show Google Sheets setup interface"""
    st.subheader("📊 การตั้งค่า Google Sheets")
    ...
    if st.session_state.sheets_manager.initialized:
        st.success("✅ เชื่อมต่อ Google Sheets สำเร็จแล้ว")
    else:
    ...
    if st.session_state.sheets_manager.initialized:
        st.markdown("---")
        st.subheader("📋 จัดการ Spreadsheet")
        ...
        with col1:
            spreadsheet_name = st.text_input(
                ...
                help="ชื่อของ Google Spreadsheet ที่จะใช้เก็บข้อมูล"
            )
...
# [Keep all other existing functions - create_enhanced_gauge_chart, display_enhanced_clo_analysis, etc.]
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Process File & Analyze", type="primary", use_container_width=True):
            with st.spinner("Processing file and performing analysis..."):
                ...
                # Step 4: Save to Google Sheets (if connected)
                if hasattr(st.session_state, 'current_spreadsheet'):
...
def create_multi_level_dashboard(results):
    ...
    # [Continue with existing dashboard code...]
...
# Main Application with improved layout and Google Sheets integration
def main():
    st.set_page_config(
        page_title="Enhanced Multi-Level Assessment System with Google Sheets",
        page_icon="🎯",
        layout="wide"
    )
    ...
    <div class="main-header">
        <h1>🎯 Enhanced Multi-Level Assessment + Google Sheets</h1>
        <p style="font-size: 1.1em;">ระบบประเมินผลการเรียนรู้แบบหลายระดับ CLO → PLO → YLO พร้อมบันทึกข้อมูลลง Google Sheets</p>
        <p style="font-size: 0.9em; opacity: 0.9;">
            📁 นำเข้าไฟล์ | 🤖 วิเคราะห์ด้วย AI | 📊 ประเมินหลายระดับ | 💾 บันทึกลง Google Sheets
        </p>
    </div>
...
และการวิเคราะห์ข้อมูลด้วยเทคนิคทางสถิติ
    ...
    with tab5:
        ...
        # PLO Information
        st.markdown("---")
        st.subheader("🎯 ผลการเรียนรู้ที่คาดหวังของหลักสูตร (PLOs)")
        
        for plo_code, plo_data in ENHANCED_PLOS.items():
            with st.expander(f"{plo_code}: {plo_data['title']} (น้ำหนัก {plo_data['weight']}%)"):
        ...
        # YLO Information
        st.markdown("---")
        st.subheader("📈 ผลการเรียนรู้ตามระดับชั้นปี (YLOs)")
        ...
        with col1:
            ...
            for ylo_code, ylo_data in year1_ylos.items():
                with st.expander(f"{ylo_code} ({ylo_data['cognitive_level']})"):
                    st.write(ylo_data['description'])
                    st.write(f"**PLO ที่เชื่อมโยง:** {', '.join(ylo_data['plo_mapping'])}")
        ...
        # Cognitive Framework
        st.markdown("---")
        st.subheader("🧠 กรอบการพัฒนาความคิด")
        
        col1, col2 = st.columns(2)
        for i, (level, data) in enumerate(COGNITIVE_FRAMEWORK.items()):
            with col1 if i < 2 else col2:
                st.markdown(f"**{level}:** {data['description']}")
                st.write(f"ตัวอย่าง: {', '.join(data['examples'][:3])}")
    ...
    with tab6:
        ...
        - **วิเคราะห์ข้อมูล**: สถิติและแนวโน้มรายวิชา
